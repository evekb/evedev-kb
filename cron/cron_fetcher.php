...
for (; $i <= $feedcount; $i++)
{
    ...
    if (preg_match($validurl , $feed[$i]))
	{
		...
		foreach($myids as $myid)
		{
			...
			if($feedlast[$i] > 0)
			{
				$out .= preg_replace('/<div.+No kills added from feed.+<\/div>/',
					'', $feedfetch->grab($feed[$i], $myid . $str, $trusted[$i],
						"fetch_url_" . $i)) . "\n";
				...
				{
					$out .= preg_replace('/<div.+No kills added from feed.+<\/div>/',
						'', $feedfetch->grab($feed[$i], $myid . $str . "&losses=1",
							$trusted[$i], "fetch_url_" . $i)) . "\n";
					if(intval($feedfetch->lastkllid_) < $lastkill || !$lastkill)
							$lastkill = intval($feedfetch->lastkllid_);
				}
				// Store most recent kill id fetched
				if($lastkill > $feedlast[$i])
				{
					$feedlast[$i] = $lastkill;
					//Fetch final kill id of board from feed if possible and set as new last kill.
					config::set("fetch_url_" . $i, $feed[$i] . ':::' . $feedlast[$i] .
						':::' . 0 . ':::' . $apikills[$i] . ':::' . $trusted[$i]);
				}
			}
			...
			else
			{
				...
				{
					$out .= preg_replace('/<div.+No kills added from feed.+<\/div>/',
						'', $feedfetch->grab($feed[$i] . "&year=" . $year . "&week=" . $l,
							$myid . $str, $trusted[$i])) . "\n";
					if(intval($feedfetch->lastkllid_) < $lastkill || !$lastkill)
							$lastkill = intval($feedfetch->lastkllid_);
					$out .= preg_replace('/<div.+No kills added from feed.+<\/div>/',
						'', $feedfetch->grab($feed[$i] . "&year=" . $year . "&week=" . $l,
							$myid . $str . "&losses=1", $trusted[$i])) . "\n";
					if(intval($feedfetch->lastkllid_) < $lastkill || !$lastkill)
							$lastkill = intval($feedfetch->lastkllid_);
				}
				// Store most recent kill id fetched
...
function getID()
{
	...
	if (config::get('cfg_pilotid'))
	{
		if(!is_array(config::get('cfg_pilotid')))
				config::set('cfg_pilotid', array(config::get('cfg_pilotid')));
	...
	elseif(config::get('cfg_corpid'))
	{
		...
		if(!is_array(config::get('cfg_corpid')))
				config::set('cfg_corpid', array(config::get('cfg_corpid')));
	...
	elseif(config::get('cfg_allianceid'))
	{
		...
		if(!is_array(config::get('cfg_allianceid')))
				config::set('cfg_allianceid', array(config::get('cfg_allianceid')));
