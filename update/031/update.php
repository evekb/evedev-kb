<?php
/**
 * re-assigns slots for all DROPPED items on existing kills
 * to valid CCP inventory flags (see update 032 for DESTROYED items)
 * 
...
function update031()
{
	...
	global $url, $smarty;
        ...
        static $TABLE_NAME = "kb3_items_dropped";
	...
	if (CURRENT_DB_UPDATE < "031") {
                ...
                else
                {
                    ...
                    $qry->execute("INSERT INTO kb3_config (cfg_site, cfg_key, cfg_value) SELECT cfg_site, 'DBUpdate', '029' FROM kb3_config GROUP BY cfg_site ON DUPLICATE KEY UPDATE cfg_value = '031'");
