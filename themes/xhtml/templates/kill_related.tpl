<!-- kill_related.tpl -->{$summarytable}
{$overview}
{$battlestats}
<div class="kb-kills-header">Battle Statistics</div>
<table class=kb-table width="100%" border="0" cellspacing=1>
	<tr class=kb-table-row-even>
		<td class=kb-table-cell width=180><b>Kills:</b></td>
		<td class=kl-kill>{$stats.kills}</td>
	</tr>
	<tr class=kb-table-row-even>
		<td class=kb-table-cell><b>Losses:</b></td>
		<td class=kl-loss>{$stats.losses}</td>
	</tr>
	<tr class=kb-table-row-even>
		<td class=kb-table-cell><b>Damage done (ISK):</b></td>
		<td class=kl-kill>{$stats.killISKB}B - {$stats.killISKM}M</td>
	</tr>
	<tr class=kb-table-row-even>
		<td class=kb-table-cell><b>Damage received (ISK):</b></td>
		<td class=kl-loss>{$stats.lossISKB}B - {$stats.lossISKM}M</td>
	</tr>
	<tr class=kb-table-row-even>
		<td class=kb-table-cell><b>Efficiency:</b></td>
		<td class=kb-table-cell><b>{$stats.efficiency}%</b></td>
	</tr>
</table>
<br/>
<div class="kb-kills-header">Related kills</div>
{$kills}
<div class="kb-losses-header">Related losses</div>
{$losses}<!-- /kill_related.tpl -->