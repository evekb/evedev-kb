        ...
        <td width=360 align=left valign=top>
            ...
            <table class="kb-table" width=360 cellpadding=0 cellspacing=1 border=0>
                <tr class="kb-table-row-even">
                    <td>
                    <img style="float: left; margin-right: 10px;" src="{if $item->item.itt_cat == 18}{$img_url}/drones/32_32/{$item->item.typeID}.png{elseif $item->item.itt_cat == 9}{$img_url}/blueprints/64_64/{$item->item.typeID}.png{elseif $item->item.itt_cat == 23 || $item->item.itt_cat == 40}{$img_url}/ships/64_64/{$item->item.typeID}.png{else}{$img_url}/items/32_32/icon{$item->item.icon}.png{/if}">
