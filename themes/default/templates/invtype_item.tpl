...
<table cellpadding="0" cellspacing="1" border="0">
    <tr>
        <td width="360" align="left" valign="top">
			...
			{if $item->item.itt_cat == 18}
			     {$img_url}/drones/64_64/{$item->item.typeID}.png
			{elseif $item->item.itt_cat == 9}
			    {$img_url}/blueprints/32_32/{$item->item.typeID}.png
			{elseif $item->item.itt_cat == 23 || $item->item.itt_cat == 40 || $item->item.itt_cat == 11 || $item->item.itt_cat == 41 || $item->item.itt_cat == 2}
			    {$img_url}/ships/64_64/{$item->item.typeID}.png
			{else}
			    {$img_url}/items/48_48/icon{$item->item.icon}.png
			{/if}" />
		    {if $item->item.itt_cat != 18}
			{if $item->attrib.techLevel.value == 2 and $item->item.itt_cat != 9}<img style="position: relative; left: -58px; float: left; margin-right: -32px;" src="{$img_url}/items/32_32/t2.png" />{/if}
		    {/if}
		    {if $item->attrib.techLevel.value == 2 and $item->item.itt_cat == 9}<img style="position: relative; left: -74px; float: left; margin-right: -10px;" src="{$img_url}/items/64_64/t2.png" />{/if}
