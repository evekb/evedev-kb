<form id="search" action="?a=admin_value_editor" method="get">
...
{section name=opt loop=$results}
  <tr class="kb-table-row-{cycle values='odd,even'}">
	...
	<td><form method='post' action='?a=admin_value_editor'>
	...
	{if $eve_central_exists eq "1"}<td><a href='?a=admin_value_editor&amp;itm_id={$results[opt].id}&amp;d=eve_central&amp;item_type={$type}'>Sync to EVE Central</a></td>{/if}
	</td>
</tr>
{sectionelse}
