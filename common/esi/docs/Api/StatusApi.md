...
# **getStatus**
> \Swagger\Client\Model\GetStatusOk getStatus($datasource, $if_none_match, $user_agent, $x_user_agent)
...
try {
    $result = $api_instance->getStatus($datasource, $if_none_match, $user_agent, $x_user_agent);
    print_r($result);
} catch (Exception $e) {
