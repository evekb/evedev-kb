...
if ($_REQUEST['opt'] == 'add')
{
	...
	foreach ($_POST['ship'] as $id => $value)
	{
		...
		$ship = new Ship($id);
		$qry->execute('insert into kb3_ships_values (shp_id, shp_value) values ('.$id.',\''.$value.'\') ON DUPLICATE KEY UPDATE shp_value = '.$value);
		$qry->execute('insert into kb3_item_price (typeID, price) values('.$ship->getExternalID().', '.$value.') ON DUPLICATE KEY UPDATE price=\''.$value.'\'');
	}
