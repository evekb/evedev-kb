...
options::fadd('Use simple URLs', 'cfg_pathinfo', 'checkbox', '', '', 'Beware, untested and may break mods. Have fun!');
...
options::fadd('Add Pilot', '', 'custom', array('admin_config', 'createPilot'));//, array('admin_config', 'reload'));
...
class admin_config
{
	...
	public static function createPilot()
	{
		...
		$plt_id = PILOT_ID;
		if(isset($_POST['option_add_pilotid']) && $_POST['option_add_pilotid'])
		{
			$_POST['option_add_pilotid'] = preg_replace("/[^0-9a-zA-Z-_.' ]/", '', $_POST['option_add_pilotid']);
		...
		else return '<input type="text" id="option_add_pilotid" name="option_add_pilotid" value="" size="40" maxlength="64" />';
		...
		{ //non-numeric
			$qry->execute("SELECT `plt_id`, `plt_name` FROM `kb3_pilots` WHERE `plt_name` like '" . $qry->escape($plt_id) . "'");
...
//		if($plt_id) $crp_id = 0;
		...
		{
			$_POST['option_add_corpid'] = preg_replace("/[^0-9a-zA-Z-_.' ]/", '', $_POST['option_add_corpid']);
		...
		else return '<input type="text" id="option_add_corpid" name="option_add_corpid" value="" size="40" maxlength="64" />';
		...
		{ //non-numeric
			$qry->execute("SELECT `crp_id`, `crp_name` FROM `kb3_corps` WHERE `crp_name` like '" . $qry->escape($crp_id) . "'");
...
//		if($plt_id || $crp_id) $all_id = 0;
		...
		{
			$_POST['option_add_allianceid'] = preg_replace("/[^0-9a-zA-Z-_.' ]/", '', $_POST['option_add_allianceid']);
		...
		{
			unset($_POST['option_add_allianceid']);
			return '<input type="text" id="option_add_allianceid" name="option_add_allianceid" value="" size="40" maxlength="64" />';
		}
		...
		{ //non-numeric
			$qry->execute("SELECT `all_id`, `all_name` FROM `kb3_alliances` WHERE `all_name` like '" . $qry->escape($all_id) . "'");
...
class update
{
	...
	public static function codeCheck()
	{
		if(!class_exists('DOMDocument'))
				return "The required DOMDocument libraries in PHP are not installed.";
		update::checkStatus();
