...
if ($_GET['op'] == 'view')
{
	...
	$html = "[<a href=\"?a=admin_cc&amp;op=add&amp;type=".$type."&amp;akey=".session::makeKey()."\">Add ".$type."</a>]<br />";
	if ($list->getCount() > 0)
	{
	...
	while ($contract = $list->getContract())
	{
		...
		$html .= "<td class='kb-table-cell' align='center' width='70'><a href=\"?a=admin_cc&amp;ctr_id=".$contract->getID()."&amp;op=edit&amp;type=".$type."&amp;akey=".session::makeKey()."\">Edit</a></td><td align='center'><a href=\"?a=admin_cc&amp;ctr_id=".$contract->getID()."&amp;op=del&amp;type=".$type."&amp;akey=".session::makeKey()."\">Delete</a></td>";
		$html .= "</tr>";
	}
	if ($list->getCount() > 0)
		$html .= "</table><br />";
	if ($list->getCount() > 10) $html .= "[<a href=\"?a=admin_cc&amp;op=add&amp;type=".$type."&amp;akey=".session::makeKey()."\">Add ".$type."</a>]";
}
// delete
if ($_GET['op'] == "del")
{
	if ($_GET['confirm'])
	{
		...
		Header("Location: ".KB_HOST."/?a=admin_cc&op=view&type=".$_GET['type']."&akey=".session::makeKey());
	}
	else
	{
		$page->setTitle("Administration - Delete ".$_GET['type']);
		$html .= "Confirm deletion:&nbsp;";
		$html .= "<button onclick=\"window.location.href='".KB_HOST."/?a=admin_cc&amp;ctr_id=".$_GET['ctr_id']."&amp;op=del&amp;type=".$_GET['type']."&amp;confirm=yes&amp;akey=".session::makeKey()."'\">Yes</button>&nbsp;&nbsp;&nbsp;";
...
if ($_GET['op'] == "edit")
{
	...
	if ($_GET['sop'])
	{
		...
		header("Location: ".KB_HOST."/?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=edit&type=".$_GET['type']."&akey=".session::makeKey());
	}
	...
	if ($_POST['add_name'])
	{
		...
		else
		{
			...
			while ($row = $qry->getRow())
			{
				...
				{
					case 0:
						$html .= "<td><a href=\"?a=corp_detail&amp;crp_id=".$row['id']."\">".$row['name']."</a></td><td align='center'><button id='submit' name='submit' onclick=\"window.location.href='".KB_HOST."/?a=admin_cc&amp;ctr_id=".$_GET['ctr_id']."&amp;op=edit&amp;type=".$_GET['type']."&amp;add_type=".$_POST['add_type']."&amp;add_id=".$row['id']."&amp;akey=".session::makeKey()."'\">Select</button></td>";
						break;
					case 1:
						$html .= "<td><a href=\"?a=alliance_detail&amp;all_id=".$row['id']."\">".$row['name']."</a></td><td align='center'><button id='submit' name='submit' onclick=\"window.location.href='".KB_HOST."/?a=admin_cc&amp;ctr_id=".$_GET['ctr_id']."&amp;op=edit&amp;type=".$_GET['type']."&amp;add_type=".$_POST['add_type']."&amp;add_id=".$row['id']."&amp;akey=".session::makeKey()."'\">Select</button></td>";
						break;
					case 2:
						$html .= "<td>".$row['name']."</td><td align=center><button id=submit name=submit onClick=\"window.location.href='".KB_HOST."/?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=edit&type=".$_GET['type']."&add_type=".$_POST['add_type']."&add_id=".$row['id']."&amp;akey=".session::makeKey()."'\">Select</button></td>";
						break;
					case 3:
						$html .= "<td>".$row['name']."</td><td align=center><button id=submit name=submit onClick=\"window.location.href='".KB_HOST."/?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=edit&type=".$_GET['type']."&add_type=".$_POST['add_type']."&add_id=".$row['id']."&amp;akey=".session::makeKey()."'\">Select</button></td>";
						break;
	...
	else
	{
		...
		$html .= "<form id=detail_edit name=detail_edit method=post action=?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=edit&type=".$_GET['type']."&akey=".session::makeKey().">";
		...
		while ($contracttarget = $contract->getContractTarget())
		{
			...
			$html .= "<td align=center><a href=\"?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=edit&type=".$_GET['type']."&sop=del_".$type."&id=".$contracttarget->getID()."&akey=".session::makeKey()."\">delete</a></td></tr>";
		}

		if ($c < 30)
		{
			$html .= "<form id=add_target name=add_target method=post action=?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=edit&type=".$_GET['type']."&akey=".session::makeKey().">";
			$html .= "<tr><td></td></tr>";
...
if ($_GET['op'] == "add")
{
	if ($_POST['detail_submit'])
	{
		...
		header("Location: ".KB_HOST."/?a=admin_cc&ctr_id=".$contract->getID()."&op=edit&type=".$_GET['type']."&akey=".session::makeKey());
	}
	...
	$html .= "<form id=detail_edit name=detail_edit method=post action=?a=admin_cc&ctr_id=".$_GET['ctr_id']."&op=add&type=".$_GET['type']."&akey=".session::makeKey().">";
