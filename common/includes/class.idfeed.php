 ...
 * 1.0.8 Add option to ignore NPC-only kills
 * @package EDK
 */
class IDFeed
{
	...
	private function processVictim($row, &$kill, $time)
	{
		...
		$kill->setVictimName($pilot->getName());
		$kill->setVictimCorpID($corp->getID());
		$kill->setVictimCorpName($corp->getName());
		$kill->setVictimAllianceID($alliance->getID());
		$kill->setVictimAllianceName($alliance->getName());
	...
	private function processInvolved($inv, &$kill, $time)
	{
		...
		$weapon = new Item((int)($inv['weaponTypeID']));
		...
		if (preg_match("/^(Mobile \w+ Warp|\w+ Control Tower( \w+)?)/",
			...
			// NPC ship
			$ship->lookup("Unknown");
			$weapon = new Item((int)($inv['shipTypeID']));
			$charname = $weapon->getName();
			$npc = true;
		}

		$pilot = new Pilot();
		$pilot->add((string)$charname, $corp, $time,
					 (int)$inv['characterID']);

		$iparty = new InvolvedParty($pilot->getID(), $corp->getID(),
				$alliance->getID(), floatval($inv['securityStatus']), $ship,
				$weapon, (int)$inv['damageDone']);

		$kill->addInvolvedParty($iparty);
		if (intval($inv['finalBlow']) == 1) {
			$kill->setFBPilotID($pilot->getID());
			$kill->setFBCorpID($corp->getID());
			$kill->setFBAllianceID($alliance->getID());
		}
